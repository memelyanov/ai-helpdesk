...
- [ ] T001 [P] Create `.gitignore` at the repository root excluding `backend/target/`,
      `frontend/node_modules/`, `frontend/.angular/`, `frontend/dist/`, `.env`, and IDE
      directories (FR-015)
- [ ] T002 [P] Create `.env.example` at the repository root with `POSTGRES_DB`, `POSTGRES_USER`
...
- [ ] T003 Copy `.env.example` to `.env` and confirm `git status` does not list it — proving T001
...
- [ ] T004 [P] [US1] Create `db/init/01-init-vector.sql` containing
      `CREATE EXTENSION IF NOT EXISTS vector;` — idempotent, and the only schema this feature
      creates (FR-002, data-model.md)
- [ ] T005 [US1] Create `docker-compose.yml` at the repository root: a single `db` service on
...
- [ ] T006 [US1] Run `docker compose up -d`, then confirm the extension query
...
- [ ] T007 [US1] Write a probe row, run `docker compose down` then `docker compose up -d`, confirm
      the row survived, then drop the probe table (scenario US1-3, FR-003, SC-004)
- [ ] T008 [US1] Confirm `docker compose down` shuts down cleanly and releases port 5432
      (scenario US1-4, FR-001)
- [ ] T009 [US1] Confirm `docker compose down -v` discards the stored state and that the init
...
- [ ] T010 [US2] Initialize the backend Maven project at `backend/`: `pom.xml` with
      ...
      Decisions 1, 2, 7)
- [ ] T011 [US2] Configure Surefire in `backend/pom.xml` with `<excludedGroups>azure</excludedGroups>`
      ...
      Decision 5)
- [ ] T012 [US2] Create `backend/src/main/java/com/epam/aihelpdesk/AiHelpdeskApplication.java` —
...
- [ ] T013 [P] [US2] `backend/src/test/java/com/epam/aihelpdesk/AiHelpdeskApplicationTests.java` —
...
- [ ] T014 [P] [US2] `backend/src/test/java/com/epam/aihelpdesk/AzureOpenAiConfigHealthIndicatorTest.java`
...
- [ ] T015 [P] [US2] `backend/src/test/java/com/epam/aihelpdesk/HealthEndpointTest.java` — the three
...
- [ ] T016 [P] [US2] `backend/src/test/java/com/epam/aihelpdesk/HealthResponseSecretTest.java` —
...
- [ ] T017 [US2] Create `backend/src/main/resources/application.yml`: datasource URL, user and
...
- [ ] T018 [US2] Create `backend/src/main/java/com/epam/aihelpdesk/health/AzureOpenAiProperties.java`
...
- [ ] T019 [US2] Create `backend/src/main/java/com/epam/aihelpdesk/health/AzureOpenAiConfigHealthIndicator.java`
...
- [ ] T020 [US2] Create `backend/src/test/java/com/epam/aihelpdesk/AzureOpenAiConnectivityIT.java`
...
- [ ] T021 [US2] With the database stopped and every `AZURE_OPEN_AI_*` variable unset in the shell,
      run `backend\mvnw.cmd test` (bash: `backend/mvnw test`) — all tests pass, **zero failures and
      zero skips** (FR-008, SC-003, SC-009)
- [ ] T022 [US2] Confirm that run made no request to Azure and that `AzureOpenAiConnectivityIT` did
...
- [ ] T025 [US3] Generate the Angular application into `frontend/` with the Angular **21.2.x** CLI,
...
- [ ] T026 [US3] Update `frontend/src/app/app.spec.ts` to assert the placeholder renders and names
      the application. **Confirm it fails** against the CLI's generated default template before
      writing T027 (FR-012)

- [ ] T027 [US3] Replace `frontend/src/app/app.html` with a placeholder page naming the application
...
- [ ] T028 [US3] Confirm the dev server binds port 4200 and, when 4200 is occupied, **fails with a
...
- [ ] T029 [US3] Run `npm test` in `frontend/` — all tests pass. Then `npm start` and confirm
...
- [ ] T030 [US4] Add the setup section to `README.md`: prerequisites with exact version ranges —
      ...
      (FR-014, SC-002)
- [ ] T031 [US4] Document `docker compose down -v` in `README.md` as a **destructive** reset that
...
- [ ] T032 [US4] State in `README.md` which shell the commands are written for (PowerShell 7+, the
...
- [ ] T033 [US4] Update the status section of `README.md` — currently "**Status: concept phase.** No
...
- [ ] T035 [P] Run the full [quickstart.md](quickstart.md) validation pass top to bottom — it is
      written as a complete manual acceptance sweep and covers scenarios these tasks reference only
      by number
- [ ] T036 [P] Secret check: `git ls-files | Select-String -Pattern '(^|/)\.env$'` (bash:
...
- [ ] T037 [P] Independence check: three separate single-part startups, each from a fully stopped
      state, matching the table in [runtime-surface.md](contracts/runtime-surface.md) (FR-013,
      SC-005)
- [ ] T038 [P] Port-conflict check: occupy 5432, 8080 and 4200 in turn and confirm each part fails
      with a message naming the occupied port rather than binding an alternative (FR-026)
- [ ] T039 Confirm no PoC behaviour was introduced anywhere in `backend/` or `frontend/` — no
